    ...
    static double compute_shift(double f, double g, double h)
    {
        ...
        if (fhmn == 0.)
        {
            ...
            if (fhmx == 0.)
            {
                ...
                d__1 = Matrix3d.min(fhmx, ga) / Matrix3d.max(fhmx, ga);
        }
        else if (ga < fhmx)
        {
            as = fhmn / fhmx + 1.;
            ...
            if (gasmal)
            {
                ...
                if (gasmal)
                {
                    ...
                    if (mm == 0.)
                    {
                        ...
                        if (l == 0.)
                            t = Matrix3d.d_sign(c_b3, ft) * Matrix3d.d_sign(c_b4, gt);
                        else
                            t = gt / Matrix3d.d_sign(d, ft) + m / t;
                    }
                    else
                        t = (m / (s + t) + m / (r + l)) * (a + 1.);
                    l = Math.sqrt(t * t + 4.);
