...
public class ClientAnimationHandler<T extends IAnimated> extends AnimationHandler<T> {
	...
	public void addAnim(String modid, String invertedAnimationName, String animationToInvert) {
		...
		if (this.animChannels.get(inverted.toString()) instanceof ClientChannel) {
			ClientChannel channel = ((ClientChannel) this.animChannels.get(inverted.toString()))
			...
			for (Entry<InfoChannel, AnimInfo> animInfo : animInfoMap.entrySet())
				if (animInfo.getKey() instanceof ClientChannel) {
					...
					int prevRotationKeyFramePosition = prevRotationKeyFrame != null
							? clientChannel.getKeyFramePosition(prevRotationKeyFrame) : 0;

					KeyFrame nextRotationKeyFrame = clientChannel.getNextRotationKeyFrameForBox(boxName,
							currentFrame);
					int nextRotationKeyFramePosition = nextRotationKeyFrame != null
							? clientChannel.getKeyFramePosition(nextRotationKeyFrame) : 0;
