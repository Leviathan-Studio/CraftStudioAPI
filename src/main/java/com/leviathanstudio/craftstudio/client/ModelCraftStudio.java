...
public class ModelCraftStudio extends ModelBase
{
    ...
    private CSModelRenderer generateCSModelRend(CSReadedModelBlock rBlock)
    {
        ...
        if (rBlock.vertex != null)
        {
            ...
            for (int i = 0; i < 8; i++)
                vertices[i] = new PositionTextureVertex(rBlock.vertex[i][0], rBlock.vertex[i][1], rBlock.vertex[i][2],
                        0.0F, 0.0F);
            modelRend.addBox(vertices, CSModelBox.getTextureUVsForRect(rBlock.texOffset[0], rBlock.texOffset[1],
                    rBlock.size.x, rBlock.size.y, rBlock.size.z));
        }
        else
            modelRend.addBox(rBlock.boxSetup.x, rBlock.boxSetup.y, rBlock.boxSetup.z, rBlock.size.x, rBlock.size.y,
                    rBlock.size.z);
